local mainMenu = RageUI.CreateMenu("", "Bahamass")
			...
			while open do
                ...
                RageUI.IsVisible(annonces, function() 
                    RageUI.Button("Annonce d'Ouverture", nil, {RightLabel = "→"}, true , {
                        onSelected = function()
                        	TriggerServerEvent('bahamas:ouvert')
                        end
                    })		
					RageUI.Button("Annonce de Fermeture", nil, {RightLabel = "→"}, true , {
    					onSelected = function()
        					TriggerServerEvent('bahamas:ferme')
    					end
					})
					RageUI.Button("Annonce Personnalisée", nil, {RightLabel = "→"}, true , {
						onSelected = function()
							...
							if notif then
								TriggerServerEvent('bahamas:perso', notif)
							else
...
Keys.Register('F6', 'bahamasjob', 'Ouvrir le menu YBahamas', function()
