                    ...
                    for k,v in pairs(all_items) do
                        RageUI.Button(v.label, nil, {RightLabel = ""..Config.Color.."x"..v.nb}, true, {
                            onSelected = function()
                                ...
                                if count <= v.nb then
                                    TriggerServerEvent('bahamas:takeStockItems', v.item, count)
                                else
                    ...
                    for k,v in pairs(all_items) do
                        RageUI.Button(v.label, nil, {RightLabel = ""..Config.Color.."x"..v.nb}, true, {
                            onSelected = function()
                                ...
                                count = tonumber(count)
                                if count <= v.nb then
                                    TriggerServerEvent("bahamas:putStockItems", v.item, count)
                                else
...
function getInventory()   
	ESX.TriggerServerCallback('bahamas:playerinventory', function(inventory)                               
		all_items = inventory
	end)
end

function getStock()
    ESX.TriggerServerCallback('bahamas:getStockItems', function(inventory)                    
        all_items = inventory
    end)
end
